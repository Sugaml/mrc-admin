...
export default function StudentPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds =  students ? students.map((n) => n.name):[];
  ...
  const filteredUsers = applySortFilter(students ? students:[], getComparator(order, orderBy), filterName);
        ...
        <Table>
          ...
          <TableBody>
            {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
              const { id, firstname, lastname,gender, is_approved,mobile_num, email, avatarUrl } = row;
              ...
              return (
                <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                  ...
                  {mobile_num}
                    {/* <Label color={(is_approved === 'true' && 'false') || 'true'}>{sentenceCase(is_approved.string)}</Label> */}
                  </TableCell>

                  <TableCell align="left">{is_approved ? 'Approved' : 'Pending'}</TableCell>
